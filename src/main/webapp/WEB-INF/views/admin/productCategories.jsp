...
<br><br>
	<div class="container">
		<h1 class="text-center"><b><i><mark>Categories List</mark></i></b></h1><br><br>
		
				<table class="table table-hover table-dark" border="1" cell padding="5">
			<thead>
				<th scope="row"><h3>Category ID</h3></th>
				<th scope="row"><h3>Category Names</h3></th>
				<th scope="row"><h3>Category Description</h3></th>
				<th scope="row"><h3>Update</h3></th>
				<th scope="row"><h3>Delete</h3></th>
				<th scope="row"><h3>Product List</h3></th>
			</thead>
			<tbody>
				<c:forEach items="${catList}" var="cat">
					<tr>
						<td><h4>${cat.id }</h4></td>
						<td><h4>${cat.categoryName }</h4></td>
						<td><h4>${cat.description }</h4></td>
						<td><spring:url value="/product/updateCategory/${cat.id}"
								var="updateURL" /> <a class="btn btn-primary"
							href="${updateURL}" role="button">Update</a></td>
						<td><spring:url value="/product/deleteCategory/${cat.id}"
								var="deleteURL" /> <a class="btn btn-primary"
							href="${deleteURL}" role="button">Delete</a></td>
						<td><spring:url value="/product/productListInCategory/${cat.id}"
								var="deleteURL" /> <a class="btn btn-primary"
							href="${deleteURL}" role="button">Product List</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		<div class="text-center">
		<spring:url value="/product/addCategory" var="addURL" />
